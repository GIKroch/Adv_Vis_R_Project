...
KOMENTARZ KK: MAMY DUZO SLAJDOW - ZOSTAWILABYM TYLKO JEDEN, SKORO PRZEDSTAWIAJA TE SAME DANE
...
KK: CHYBA ZE CHCEMY WYKRES LINIOWY DLA UROZMAICENIA? DO PRZEMYSLENIA (zrobilam go przez przypadek XD)
...
century_basic$positive <- pos_all
century_basic$neutral <- neutr_all
century_basic$negative <- neg_all

positivity <- century_basic %>% melt(id=c("centuries")) %>% filter(variable %in% c("positive", "negative"))
...
ggplot(data = positivity, aes(x = centuries, y = value, group = variable)) +
  ...
  geom_label_repel(label.size=NA,
                   aes(label = variable),
                  nudge_x = 1,
                  na.rm = TRUE, data = positivity %>% filter(centuries=="20th"))
